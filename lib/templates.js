...
export function buildComplaintDoc(d) {
  ...
  return `<div class="doc">
    ...
    <table>
      ...
      <tr><th>الرقم الآلي</th><th>المحطة</th><th>UDS</th></tr>
      <tr><td class="val">${escapeHtml(d.paci)}</td><td class="val">${escapeHtml(d.station)}</td><td class="val">${escapeHtml(d.uds)}</td></tr>
      <tr><th colspan="3">اليونت</th></tr>
      <tr><td class="val" colspan="3">${escapeHtml(d.unitNo)}</td></tr>
...
export function buildRepeatedStationsDoc(entries) {
  const sections = entries.map((entry, i) => {
    ...
    const unitLabel = entry.unit ? ` — اليونت: ${entry.unit}` : '';
    return `<div class="box-label" style="font-size:22px;margin-top:${i === 0 ? '0' : '20px'};">${i + 1}. ${escapeHtml(entry.fieldLabel)}: ${escapeHtml(entry.value)}${unitLabel}${areasLabel} — (${entry.count} بلاغ)</div>
