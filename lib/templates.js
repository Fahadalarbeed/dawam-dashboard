...
export function buildComplaintDoc(d) {
  ...
  return `<div class="doc">
    ...
    <table>
      ...
      <tr><th>الرقم الآلي</th><th>المحول / UDS</th><th>اليونت</th></tr>
...
export function buildRepeatedStationsDoc(entries) {
  const sections = entries.map((entry, i) => {
    const rows = entry.items
      ...
      .join('');
    return `<div class="box-label" style="font-size:22px;margin-top:${i === 0 ? '0' : '20px'};">${i + 1}. محطة/UDS: ${escapeHtml(entry.station)} — (${entry.count} بلاغ)</div>
