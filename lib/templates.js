...
export function buildRepeatedStationsDoc(entries) {
  const sections = entries.map((entry, i) => {
    ...
    return `<div class="box-label" style="font-size:22px;margin-top:${i === 0 ? '0' : '20px'};">${i + 1}. ${escapeHtml(entry.fieldLabel)}: ${escapeHtml(entry.value)}${areasLabel} — (${entry.count} بلاغ)</div>
