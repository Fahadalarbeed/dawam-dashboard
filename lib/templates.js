...
export function buildComplaintDoc(d) {
  ...
  return `<div class="doc">
    ...
    <table>
      ...
      <tr><th>الشارع</th><th>المنزل</th><th>الرقم الآلي</th></tr>
      <tr><td class="val">${escapeHtml(d.street)}</td><td class="val">${escapeHtml(d.house)}</td><td class="val">${escapeHtml(d.paci)}</td></tr>
      <tr><th>المحول / UDS</th><th>اليونت</th><th>رقم الهاتف</th></tr>
      <tr><td class="val">${escapeHtml(d.station)}</td><td class="val">${escapeHtml(d.unitNo)}</td><td class="val">${escapeHtml(d.phone)}</td></tr>
    </table>
