...
export async function uploadReportPhoto(id, kind, blob) {
  const ext = blob.type === 'image/png' ? 'png' : 'jpg';
  const path = `photos/${id}-${kind}.${ext}`;
  const { error } = await supabase.storage.from(BUCKET).upload(path, blob, {
...
export async function downloadReportPhoto(photoPath) {
...
export async function updateReportData(id, dataPatch) {
  const { data: existing, error: fetchErr } = await supabase.from('reports').select('data').eq('id', id).single();
  if (fetchErr) throw fetchErr;
  const mergedData = { ...(existing.data || {}), ...dataPatch };
  const { data, error } = await supabase.from('reports').update({ data: mergedData }).eq('id', id).select().single();
  if (error) throw error;
  return data;
}

// from/to are 'YYYY-MM-DD' strings (inclusive)
export async function searchReports({ from, to, type, area, block, street, building, house, paci, meterNo, action, phone, station }) {
  let query = supabase
    .from('reports')
    .select('*')
  ...
  if (phone) query = query.ilike('data->>phone', `%${phone}%`);
  // one box searches both fields, since a complaint carries either a station or a UDS
  if (station) query = query.or(`data->>station.ilike.%${station}%,data->>uds.ilike.%${station}%`);
