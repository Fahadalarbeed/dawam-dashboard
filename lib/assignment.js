...
// Added to the score of technicians we cannot locate, so a confirmed-nearby
// technician always wins over one whose position is a guess.
const UNKNOWN_LOCATION_PENALTY_KM = 12;
...
export function autoAssignDriver(complaints, area, liveLocations = {}) {
  ...
  const scored = eligible.map((d) => {
    ...
    let dist;
    if (live !== null) {
      // Real GPS position — the only thing we actually know.
      dist = live;
    } else {
      // No live signal: fall back to the last area worked. A technician whose
      // position is unknown must never outrank one we can see nearby, so the
      // fallback carries a penalty instead of counting as distance zero.
      const last = lastAreaFor(complaints, d);
      dist = (last ? areaDistanceKm(area, last) : 25) + UNKNOWN_LOCATION_PENALTY_KM;
    }
